            ...
            $('#employeestable tbody').on('click', '.mony', function() {
                ...
                Swal.fire({
                    ...
                    preConfirm: () => {
                        ...
                        Swal.showLoading()
                        helperjs('{{ route('discount.storedebit') }}', data)
                        $('#employeestable').DataTable().draw();
                    },
                    allowOutsideClick: () => !Swal.isLoading()
                })
            });
            $('#employeestable tbody').on('click', '.discount', function() {
                let id = $(this).attr("value");
                Swal.fire({
                    title: ' اضافة خصم',
                    html: '<input id="swal-input1" class="swal2-input" type="number">' +
                    ...
                    preConfirm: () => {
                        ...
                        Swal.showLoading()
                        helperjs('{{ route('discount.storediscount') }}', data)
                        $('#employeestable').DataTable().draw();
                    },
                    allowOutsideClick: () => !Swal.isLoading()
                })
            });
            $('#employeestable tbody').on('click', '.mony2', function() {
                let id = $(this).attr("value");
                Swal.fire({
                    title: ' اضافة حافز او مكأفاه',
                    html: '<input id="swal-input1" class="swal2-input" type="number">' +
                        '<input id="swal-input2" class="swal2-input" type="date">',
                    focusConfirm: false,
                    showCancelButton: true,
                    ...
                    preConfirm: () => {
                        ...
                        Swal.showLoading()
                        helperjs('{{ route('rewards.store') }}', data)
                        $('#employeestable').DataTable().draw();
                    },
                    allowOutsideClick: () => !Swal.isLoading()
                })
